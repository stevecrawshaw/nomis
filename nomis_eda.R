#EDA script using the deprecated nomisr package to introspect some
# sample data for age and sex in the four WECA authorities
# and create an age \ sex pyramid
...
woe_la_codes_tbl <- nomis_get_metadata(
  sex_by_age_id,
  concept = "GEOGRAPHY",
  type = "TYPE423",
  time = "latest"
  ...
  filter(
    label.en |> str_detect("Bristol|North Somerset|Bath|South Gloucestershire")
  ) |>
  glimpse()

woe_la_geogs <- woe_la_codes_tbl |> pull(id)

nomis_lsoa <- nomis_get_metadata(
  age_id,
  concept = "GEOGRAPHY",
  type = "TYPE151",
  time = "latest"
...
sex_by_age_raw_tbl <- nomis_get_data(
  id = sex_by_age_id,
...
pyramid_plot_tbl <- sex_by_age_raw_tbl |>
  ...
  filter(c_2021_age_24_name != "Total", c_sex_name != "All persons") |>
  mutate(
    c_2021_age_24_fct = str_remove(c_2021_age_24_code, "_") |>
      as.integer(),
  ...
  summarise(obs_value = sum(obs_value), order_fct = mean(c_2021_age_24_fct)) |>
  glimpse()
...
pyramid_plot <- pyramid_plot_tbl |>
  ...
  labs(
    ...
    caption = glue(
      "Derived from NOMIS dataset RM121 \ ({sex_by_age_id}): 2021 census data."
    )
  ) +
...
age_sex_ods_lep_tbl <- pyramid_plot_tbl |>
  mutate(
    mid_range_int = map_int(five_year_bands, ~ make_mid_int(.x)),
    order_fct = NULL,
    geography_name = str_remove(geography_name, ", City of")
  ) |>
  rename(
    "age_band" = five_year_bands,
